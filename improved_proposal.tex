...
\draw[arrow] (fail) -| node[below, font=\footnotesize, pos=0.3] {Auto-select} (fallback);
\draw[arrow] (success) -- ++(0,-2.5) -| node[below, font=\footnotesize, pos=0.2] {Promote} (fallback);
...
This project will adapt this mechanism for UKI fallback selection, eliminating the need for manual intervention after a failed boot. The \texttt{qubes-uki-promote-fallback} command will still exist for explicit promotion of a known-good UKI to fallback status.
