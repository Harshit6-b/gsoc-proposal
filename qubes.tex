...
\begin{enumerate}
    ...
    \item \textbf{Fallback Recovery (\texttt{qubes-uki-promote-fallback}):} A fallback UKI \\ (\texttt{qubes-fallback.efi}) built from a last-known-good snapshot, registered as a lower-priority UEFI boot entry; includes automatic fallback selection on boot failure and manual promotion command
...
\node[block, fill=securegreen!20, below=1.5cm of primary] (fallback) {\textbf{qubes-fallback.efi}\\(Fallback)};

% Boot counter mechanism
\node[block, fill=warnorange!20, right=2cm of primary] (counter) {boot\_counter\\decrements};
\node[block, fill=securegreen!30, right=2cm of counter] (success) {boot\_success=1\\(systemd sets)};
...
\draw[arrow, dashed, gray] (success) -- ++(0,-2.5) -| node[below, font=\footnotesize, pos=0.2, text=gray] {Manual promote} (fallback);

\end{tikzpicture}
\caption{Automatic fallback using GRUB boot\_counter/boot\_success variables. Failed boots automatically trigger fallback selection.}
\label{fig:fallback}
\end{figure}

\textbf{Leveraging Fedora's boot\_success mechanism:} Fedora implements automatic boot failure detection using GRUB environment variables (\texttt{boot\_counter} and \texttt{boot\_success}), originally developed for the \href{https://fedoraproject.org/wiki/Changes/HiddenGrubMenu}{Hidden GRUB Menu} feature. The mechanism works as follows:

\begin{enumerate}
    \item On boot, GRUB decrements \texttt{boot\_counter}
    \item If boot succeeds, a systemd service sets \texttt{boot\_success=1} and resets the counter
    \item If \texttt{boot\_counter} reaches 0 without success, GRUB automatically selects the fallback entry
\end{enumerate}

This project will adapt this mechanism for UKI fallback selection, eliminating the need for manual intervention after a failed boot.

\textbf{Note on current Qubes configuration:} Qubes currently disables Fedora's boot\_counter mechanism in its GRUB configuration. Before re-enabling this for UKI fallback, I will need to:
\begin{enumerate}
    \item Investigate \textit{why} Qubes disabled this mechanism (potential reasons include: conflicts with Xen-specific boot flow, user expectation mismatches, or issues with the hidden GRUB menu behavior)
    \item Determine whether the original concerns apply to the UKI boot path
    \item Identify the specific configuration changes required (e.g., modifying relevant GRUB and systemd configuration, adjusting \texttt{/etc/default/grub} settings, or adapting the mechanism for EFI boot entries rather than GRUB menu entries)
\end{enumerate}
This investigation is scheduled for community bonding and Week 7, with findings documented before implementation.

The \texttt{qubes-uki-promote-fallback} command will still exist for explicit promotion of a known-good UKI to fallback status.
...
Finalise \texttt{secureboot.conf} schema with Marek. Set up QEMU + OVMF test environment. Submit \texttt{qubes-pecheck} patches. Investigate why Qubes disables boot\_counter mechanism. Review \texttt{qubes-vmm-xen-unified} build scripts. \\
\midrule
...
\textbf{qubes-core-agent-linux \href{https://github.com/QubesOS/qubes-core-agent-linux/pull/644}{PR \#644}:} Approved, pending merge.
